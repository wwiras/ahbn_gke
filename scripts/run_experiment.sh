...
if [ -z "$IMAGE" ]; then
  ...
  echo 'IMAGE=wwiras/ahbn-peer:v3 ./scripts/run_exp10.sh'
  exit 1
fi

NUM_NODES="$(python - <<PY
import yaml
cfg = yaml.safe_load(open("${ROOT_DIR}/${CONFIG}"))
print(int(cfg.get("numNodes", 20)))
PY
)"
...
# if [ -z "$IMAGE" ]; then
#   echo "ERROR: set IMAGE, e.g."
#   echo 'IMAGE=yourdockerhub/ahbn-peer:v3 ./scripts/run_exp10.sh'
#   exit 1
# fi
...
#   kubectl -n "${NAMESPACE}" get statefulset peer > "${OUTDIR}/statefulset.txt" 2>/dev/null || true
#   kubectl -n "${NAMESPACE}" describe statefulset peer > "${OUTDIR}/statefulset_describe.txt" 2>/dev/null || true
#   kubectl -n "${NAMESPACE}" describe pod peer-35 > "${OUTDIR}/peer35_describe.txt" 2>/dev/null || true
#   kubectl -n "${NAMESPACE}" logs job/ahbn-controller > "${OUTDIR}/controller.log" 2>/dev/null || true
...
# echo "[7] Start controller job"
# helm upgrade "${RELEASE}" "${ROOT_DIR}/helm/ahbn" \
#   --namespace "${NAMESPACE}" \
#   --reuse-values \
#   --set controller.enabled=true

# echo "[8] Wait for controller completion"
# kubectl -n "${NAMESPACE}" wait \
#   --for=condition=complete job/ahbn-controller \
#   --timeout=600s
